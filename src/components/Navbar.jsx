...
const Navbar = ({ currentPage, navigate, introDone }) => {
  ...
  /* ── Wait for intro to finish, then stagger links in ── */
  useEffect(() => {
    if (!introDone) return;
        ...
        <div className="nav-container">
          ...
          <ul className="nav-links">
            ...
            <motion.li
              initial={{ opacity: 0, y: -12 }}
              animate={showItems ? { opacity: 1, y: 0 } : { opacity: 0, y: -12 }}
              transition={{ delay: 0.1 + NAV_LINKS.length * 0.09, duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
            >
      ...
      <div className={`nav-drawer ${menuOpen ? 'open' : ''}`}>
        ...
        <div className="drawer-cta-wrap">
          ...
          <div className="drawer-contact-info">
            <a href="tel:+919876543210" className="drawer-contact-link">📞 +91 98765 43210</a>
            <a href="mailto:info@leonatech.in" className="drawer-contact-link">✉️ info@leonatech.in</a>
          </div>
        </div>
        ...
        <div className="drawer-decoration" aria-hidden="true">
          <svg viewBox="0 0 300 200" fill="none">
            ...
            <path d="M0,80 C60,50 120,110 180,70 S260,90 300,60" stroke="rgba(13,110,110,0.1)" strokeWidth="1.5" fill="none"/>
          </svg>
        </div>
      </div>
    </>
